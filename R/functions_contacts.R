...
#'
#' @examples
#' AddContactRequest$new(inputUser, "John", "Doe", "+123", TRUE)$to_list()
...
#'
#' @examples
#' BlockRequest$new(peerObj, TRUE)$to_list()
...
#'
#' @examples
#' BlockFromRepliesRequest$new(123, TRUE, FALSE, TRUE)$to_list()
...
#'
#' @examples
#' GetBlockedRequest$new(0, 100, TRUE)$to_list()
...
#'
#' @examples
#' GetContactIDsRequest$new(0)$to_list()
...
#'
#' @examples
#' GetContactsRequest$new(0)$to_list()
...
#'
#' @examples
#' GetLocatedRequest$new(geoPointObj, TRUE, 60)$to_list()
...
#'
#' @examples
#' GetSponsoredPeersRequest$new("query")$to_list()
...
#'
#' @examples
#' GetStatusesRequest$new()$to_list()
...
#'
#' @examples
#' GetTopPeersRequest$new(0, 10, 0)$to_list()
...
#'
#' @examples
#' ImportContactTokenRequest$new("tok")$to_list()
...
#'
#' @examples
#' ImportContactsRequest$new(list(contact1, contact2))$to_list()
...
#'
#' @examples
#' ResetTopPeerRatingRequest$new(categoryObj, peerObj)$to_list()
...
#'
#' @examples
#' ResolvePhoneRequest$new("123456")$to_list()
...
#'
#' @examples
#' ResolveUsernameRequest$new("bob", "ref")$to_list()
...
#' @field SUBCLASS_OF_ID integer Subclass id (hex)
#' @field q character Query string
#' @field limit integer Result limit
#'
#' @description
#' Methods:
...
SearchRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @return list
    to_list = function() {
      list(`_` = "SearchRequest", q = self$q, limit = self$limit)
    },
    ...
    #' @return raw
    to_bytes = function() {
      prefix <- as.raw(c(0xd8, 0x12, 0xf8, 0x11))
      q_bytes <- private$serialize_string_tl(self$q)
      limit_raw <- private$int_to_le_raw(as.integer(self$limit), size = 4L)
      c(prefix, q_bytes, limit_raw)
    }
  ),
  private = list(
    #' Convert integer to little-endian raw vector of given size (bytes)
...
#' Create SearchRequest from reader
#'
#' @param reader object with methods tgread_string() and read_int()
#' @return SearchRequest
SearchRequest$from_reader <- function(reader) {
  q_val <- reader$tgread_string()
  limit_val <- reader$read_int()
  SearchRequest$new(q = q_val, limit = limit_val)
}
...
#'
#' @examples
#' SetBlockedRequest$new(list(peer1, peer2), 10, TRUE)$to_list()
...
#'
#' @examples
#' ToggleTopPeersRequest$new(TRUE)$to_list()
...
#'
#' @examples
#' UnblockRequest$new(id = somePeer, my_stories_from = TRUE)$to_list()
