---
title: "Channel Interactions: Members, Replies, and Reactions"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Channel Interactions: Members, Replies, and Reactions}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

## Goal

This short case study shows how to:

1. Download channel members.
2. Fetch replies (comments) to channel posts.
3. Summarize reactions by emoji.

## Offline Demo (No Telegram Connection)

The outputs below are generated from a bundled sample dataset to keep the vignette reproducible.

```{r sample_data, include=FALSE}
sample <- readRDS(system.file("extdata/vignettes/channel_sample.rds", package = "telegramR"))
members_sample <- sample$members
replies_sample <- sample$replies
reactions_sample <- sample$reactions
library(telegramR)
library(dplyr)
library(stringr)
library(ggplot2)
```

## Setup

```{r setup, eval=FALSE}
library(telegramR)
library(dplyr)
library(stringr)
library(ggplot2)

api_id <- 123456
api_hash <- "0123456789abcdef0123456789abcdef"

client <- TelegramClient$new("my_session", api_id, api_hash)
client$start()
```

## 1) Members (Participants)

```{r members, eval=FALSE}
members <- download_channel_members(
  client,
  "V_Zelenskiy_official",
  limit = 500
)

members %>%
  count(is_bot, sort = TRUE)
```

### Example Output (Bundled Sample)

```{r members_sample_output}
members <- members_sample
members %>%
  count(is_bot, sort = TRUE)
```

## 2) Replies (Comments)

```{r replies, eval=FALSE}
replies <- download_channel_replies(
  client,
  "V_Zelenskiy_official",
  message_limit = 20,
  reply_limit = Inf
)

replies %>%
  count(from_type, sort = TRUE)
```

### Example Output (Bundled Sample)

```{r replies_sample_output}
replies <- replies_sample
replies %>%
  count(from_type, sort = TRUE)
```

## 3) Reactions Breakdown

```{r reactions, eval=FALSE}
reactions <- download_channel_reactions(
  client,
  "V_Zelenskiy_official",
  limit = 200
)

emoji_counts <- reactions %>%
  filter(!is.na(reactions_json)) %>%
  mutate(reactions_json = ifelse(reactions_json == "{}", NA_character_, reactions_json)) %>%
  filter(!is.na(reactions_json))

emoji_counts
```

### Example Output (Bundled Sample)

```{r reactions_sample_output}
reactions <- reactions_sample
emoji_counts <- reactions %>%
  filter(!is.na(reactions_json)) %>%
  mutate(reactions_json = ifelse(reactions_json == "{}", NA_character_, reactions_json)) %>%
  filter(!is.na(reactions_json))

emoji_counts
```
