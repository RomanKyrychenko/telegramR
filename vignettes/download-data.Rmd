...
This vignette demonstrates how to download data from Telegram channels using the `telegramR` package. The `telegramR` package provides an R6 client interface similar to Python's Telethon.

## Setup and Authentication

First, you need to load the package and instantiate a `TelegramClient`. You will need your `api_id` and `api_hash` which you can get from [my.telegram.org](https://my.telegram.org).
...
Before downloading messages, you need to resolve the channel entity. You can do this using the channel's username (e.g., `@RData`) or invite link.

```{r get_entity, eval=FALSE}
...
To download messages from the channel, you can use the `get_messages` or `iter_messages` methods.

### Getting Recent Messages

If you just want a specific number of recent messages, use `get_messages`:
...
If you want to download the entire history of a channel, it is recommended to use `iter_messages`. Note that downloading thousands of messages might take time due to Telegram's API limits.

```{r iter_all, eval=FALSE}
...
If the messages contain media (such as photos or documents), you can download them using `download_media`:
...
Once you have finished downloading the data, ensure that you disconnect the client to safely close the session:
